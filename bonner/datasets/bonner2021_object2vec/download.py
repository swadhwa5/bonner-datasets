...
from .constants import N_SUBJECTS, URLS, FILENAMES


def download_dataset(force_download: bool, **kwargs) -> None:
    filepath = download_file(
        URLS["stimuli"], filepath=Path(FILENAMES["stimuli"]), force=force_download,
    )
    unzip_file(filepath, extract_dir=Path.cwd(), remove_zip=False)

    download_file(URLS["conditions"], filepath=Path(FILENAMES["conditions"]), force=force_download)
